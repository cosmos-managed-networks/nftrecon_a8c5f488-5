...
# then log output of file /code/bin/nftrecond
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/nftrecond \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/nftrecond | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/nftrecond /usr/bin/nftrecond
