...
HTTPS_GIT := github.com/nftrecon/nftrecon.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=nftrecon \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=nftrecond \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/nftrecond ./cmd/nftrecond
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/nftrecond.exe ./cmd/nftrecond
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/nftrecond
...
draw-deps:
	...
	@goviz -i ./cmd/nftrecond -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c nftrecon --local -f chains.yaml
endif
...
set-testnet-configs:
	nftrecond config set client chain-id chainid-1
	nftrecond config set client keyring-backend test
	nftrecond config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | nftrecond keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | nftrecond keys add acc1 --recover`
